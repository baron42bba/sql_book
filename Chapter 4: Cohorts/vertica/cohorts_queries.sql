...
SELECT AGE_IN_YEARS(b.term_start,a.first_term) as periods
...
FROM
(
        SELECT AGE_IN_YEARS(b.term_start,a.first_term) as period
        ,count(distinct a.id_bioguide) as cohort_retained
        FROM
        (
                SELECT id_bioguide
...
(
        ...
        (
                SELECT 
                AGE_IN_YEARS(b.term_start,a.first_term) as period
...
,c.date
,AGE_IN_YEARS(c.date,a.first_term) as period
FROM
(
...
SELECT coalesce(AGE_IN_YEARS(c.date,a.first_term),0) as period
,count(distinct a.id_bioguide) as cohort_retained
FROM
(
        SELECT id_bioguide, min(term_start) as first_term
...
,coalesce(AGE_IN_YEARS(c.date,a.first_term),0) as period
,count(distinct a.id_bioguide) as cohort_retained
FROM
(
        SELECT id_bioguide, min(term_start) as first_term
